	...
	<tr>
		<td colspan="2">
			<ul>
				...
				<li>
					<b><?php echo esc_html__('Dispatch Date', 'Print-Google-Cloud-Print-GCP-WooCommerce'); ?>:</b> <?php echo esc_html(date(get_option('date_format', 'm/d/Y'))); ?>
				</li>
	...
	<?php if ($location_data['total']['cost']) { ?>
		...
		<?php if ($order->get_meta('pos-tip')) { ?>
			<tr>
				...
				<td><?php wp_kses_post(wc_price($order->get_meta('pos-tip'), array('currency' => $order->get_currency()))); ?></td>
			</tr>
		<?php } ?>
		<?php if ($order->get_meta('pos-cash-tendered')) { ?>
			<tr>
				...
				<td><?php echo wp_kses_post(wc_price($order->get_meta('pos-cash-tendered'), array('currency' => $order->get_currency()))); ?></td>
			</tr>
		<?php } ?>
		<tr class="total">
			<td colspan="4"></td>
