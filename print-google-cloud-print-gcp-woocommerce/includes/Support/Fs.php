...
class Fs {
	public static function get_url( string $rel = '', bool $stamp = false ): string {
		$url = plugin_dir_url( PLUGIN_ROOT_FILE );
	...
	public static function get_path( string $rel = '' ): string {
		$path = plugin_dir_path( PLUGIN_ROOT_FILE );
	...
	public static function write( string $path, string $content, int $permissions = 0600 ): bool {
		...
		if ( ! $output ) {
			Notice::add_transient( sprintf( __( 'Can\'t write the %s.', 'Print-Google-Cloud-Print-GCP-WooCommerce' ), $path ), 'error' );
	...
	public static function read( string $path ) {
		if ( ! file_exists( $path ) ) {
			Notice::add_transient( sprintf( __( 'Can\'t read the %s.', 'Print-Google-Cloud-Print-GCP-WooCommerce' ), $path ), 'error' );
